        ...
        <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth sx={{ fontFamily: 'Roboto, Arial, sans-serif' }}>
            <DialogContent>
                <Box sx={{ display: 'flex', gap: 2, p: 1 }}>
                    ...
                    <Typography variant="h5" fontWeight="bold" sx={{ mt: 1, color: '#616161' }}>{t('excluir')}</Typography>
                </Box>
                <Box sx={{ display: 'flex', gap: 1, p: 1, mt: 2 }}>
                    <Typography variant="h6" fontWeight="400" sx={{ mb: 2 }}>{t('temcertezaque')}</Typography>
                    <Typography variant="h6" fontWeight="bold" sx={{ mb: 2 }}>{fruta.fruta}?</Typography>
                </Box>
                <DialogActions sx={{ display: 'flex', gap: 1 }}>
                    <Button onClick={onClose} sx={{ fontFamily: 'Roboto', backgroundColor: 'transparent', color: 'black'}}>{t('cancelar')}</Button>
                    <Button onClick={onConfirm} sx={{ fontFamily: 'Roboto', backgroundColor: 'error.main', color: 'white'}}>{t('excluir')}</Button>
                </DialogActions>
            </DialogContent>
        </Dialog>
    );
